...
\item From schemas $\schema$ and $\schema'$, a migration engineer%
\footnote{The migration engineer may also stand for a team, which may overlap with the developer role.}
      generates a migration script, using the Ampersand compiler.
      This script specifies the migration system, $\migrsys$, that includes $\infsys$, $\infsys'$ and a number of migration rules.
      $\migrsys$ is meant to migrate the data and ensure that $\dataset'$ satisfies all invariants of $\schema'$.
\item\label{item3}
      In specific cases, the schema $\schema_{\migrsys}$ will be enough to perform the migration automatically.
      As soon as the data is transferred and $\migrsys$ has satisfied the invariants of $\schema'$,
      $\infsys'$ can take care of production on its own without the help of $\migrsys$.
      However, a migration engineer may have specific requirements that call for changes in $\migrsys$.
      For that purpose, she may want to alter the migration script to accommodate such requirements.
      ...
      But she can also ask business users to resolve issues by specifying process rules.
...
\item\label{item4}
      Then, the migration engineer deploys system $\migrsys'$.
\begin{figure}
   \centering
   \includegraphics[scale=0.1]{figures/4MoD.png}
   \caption{During the migration}
   \label{fig:4MoD}
\end{figure}
      Let us call this moment the moment of deployment (MoD).
      ...
      During this phase, users can still change $\dataset$ (both inserts and deletes) while $\infsys$ is keeping its invariants true.
...
\begin{figure}
   \centering
   \includegraphics[scale=0.1]{figures/5MoT.png}
