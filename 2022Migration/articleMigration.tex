...
\address[ou]{Open Universiteit, Heerlen, the Netherlands}
...
\begin{abstract}
   ...
   The migration process is based on the assumption that
   ...
   the business continues during the migration without interruption (zero downtime),
...
\footnote{These effects can be measured in terms of DevOps metrics such as
   ...
   This paper proposes a theory for data migrations in general. 
   It will be implemented in a software generator called Ampersand~\cite{JoostenRAMiCS2017, Joosten-JLAMP2018},
   but the theory is kept general to make it applicable in other contexts as well.
   It aims at automating data migrations, so developers can change systems in production more reliably and faster.
   This contributes to a more agile software development process.
   We believe that end users will experience a more organic evolution of the system because increments are smaller, occur more frequently and more reliably.
...
\subsection{Information Systems}
   Our analysis of data migration starts with a definition of information systems.
   This comprises a schema, which contains all information that makes data meaningful.
   ...
   These users work from different locations and on different moments. They work with data that the system keeps (syn: stores, maintains).
   As a consequence, the data in a system changes continually.
   Other computers can use the system too. So, we will use the word ``actor'' to refer to both users and computers.
   The collective use by multiple actors serves a purpose which we will loosely call ``the business''.
   It is this business that relies on the semantics of the data for achieving its goals and fulfilling its purpose.

   Besides data, the system also contains services that actors can use to change the data.
   For the data migration we need services that can be switched on and off independently of each other,
   so we can switch old and new functionalities on and off as needed in the process of migrating the data.
   ...
   We need the ingress function during a migration to switch users from the old to new functionality.
   ...
   In this work, we represent the semantics of an information system as a set of constraints on the data in the system.
   Every constraint a service keeps satisfied is called an invariant.
   So, deploying a service means that the system will keep the invariants of that service satisfied as long as it is alive.
   It also means that the invariants of a service are prerequisites for its deployment.
   These constraints are part of the schema of the information system.
   The data contained in a system represents the state of that system.
...
\end{figure}
   ...
   To try out and implement our theory, we use a software generator called Ampersand~\cite{JoostenRAMiCS2017, Joosten-JLAMP2018}.
   Like most other software generators, Ampersand is a compiler that generates information systems from a script.
   The choice for Ampersand is motivated by its proximity to the theory in this paper, which makes it easy to implement the theory.
...
\footnote{The dotted lines in the diagrams show which set of rules is enforced.}.
   Ampersand generates all program code needed in the system from a script, the Ampersand script,
   in which a developer only specifies rules (semantic constraints). The generator does the rest of the programming.
   So, developers need notag extract constraints from specifications or (even more laborious) from program code
   because they have already specified every constraint explicitly as a rule in the Ampersand script.

   Our theory can be implemented in an ordinary programming language as well, although it might require more effort.
...
\end{figure}
   ...
   so the migration engineer can change everything she\footnote{Throughout this paper, we ask the reader to read ``she'' or ``her'' to include the masculin form as well.} wants to suit the particulars of the data migration.
