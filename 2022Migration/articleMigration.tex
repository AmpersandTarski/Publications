...
\begin{abstract}
   ...
   However, many generators do not support data migrations, which are necessary when the schema of a system changes during its operational life.
   As a consequence, developers often migrate data ``by hand'', which slows down the pace of incremental software development.

   To address this problem, this paper proposes a theory for data migrations.
...
\footnote{In the sequel, the word ``system'' refers to the phrase ``information system''. This simplifies the language a little. }.
   ...
   Developers can use their efforts saved by generating systems to decrease the time to release successive increments.
   More frequent data migrations are then to be expected.
   ...
   With such generators at hand, we can expect developers to spend a larger portion of their time on data migrations,
   which limits the pace of deploying successive releases%
\footnote{These effects can be measured in terms of DevOps metrics such as
   ...
   This paper proposes a theory for data migrations that is meant to be implemented in a software generator called Ampersand~\cite{Joosten-JLAMP2018, JoostenRAMiCS2017}.
...
\end{figure}
   ...
   Like most other software generators, Ampersand~\cite{Joosten-JLAMP2018, JoostenRAMiCS2017}
   is a compiler that generates information systems from a script.
   Once a generated system is in production, changes that affect the schema are expensive because developers must migrate data ``by hand''.
   This is an error-prone process which slows down the deployment of successive increments.
...
\subsection{Data Migrations}
   ...
   A migration strategy (e.g. case by case, customer by customer, or in batches) must ensure the quality of the migration.
   To assume that this can be done fully automatically is unrealistic in many cases%
...
%\end{lemma}
%   This lemma is proven in Isabelle/HOL~\cite{Isabelle}. % TODO: point to formalization %The proof is published \href{location.domain}{here}
%
...
\end{eqnarray}
   ...
   An enforcement rule with operator {\tt :<} (alternative~\ref{enforce del}) keeps the population of $r$ smaller than or equal to \id{expression}.
   ...
   An enforcement rule with operator {\tt >:} (alternative~\ref{enforce ins}) keeps the population of $r$ larger than or equal to \id{expression}.
   ...
   An enforcement rule with operator {\tt :=} (alternative~\ref{enforce eq}) keeps the population of $r$ equal to \id{expression} by inserting and deleting the right pairs in $r$.
...
\begin{verbatim}
   ...
   RULE ExamRule1: new.student~;new.course |- new.takes
   RULE ExamRule2: new.student~;new.course;new.isPartOf~ 
                 |- new.isEnrolledFor
\end{verbatim}
