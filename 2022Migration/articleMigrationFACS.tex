...
\end{align}
   ...
   When clarity is needed, we write $\concepts_{\schema}$, $\rels_{\schema}$, $\rules_{\schema}$, $\transactions_{\schema}$, and $\busConstraints_{\schema}$
   for $\concepts$, $\rels$, $\rules$, $\transactions$, and $\busConstraints$ corresponding to $\schema = \quintuple{\concepts}{\rels}{\rules}{\transactions}{\busConstraints}$.
...
\subsection{Generating a migration script}
   ...
   We use a prime symbol to denote the elements of the desired system, so $\infsys$ is the existing system and $\infsys'$ is the desired system.
   We also have to relabel items with prefixes to avoid name clashes in the migration system.
...
\item Let us combine the above into a single migration schema:
   \begin{align}
      \schema_\migrsys=\quintuple{\concepts_\dataset\cup\concepts_{\dataset'}}{\overleftarrow\rels_{\schema}\cup\overrightarrow\rels_{\schema'}\cup\rels_3}{\rules_\text{block}}{\transactions_1\cup\transactions_2}{\busConstraints_\text{fix}\cup\busConstraints_{\schema'}}
   \end{align}
\end{enumerate}
