...
\label{sct:Introduction}
   ...
   The ubiquitous pressure to decrease deploy times and increase release frequencies calls for further automation of SCDMs.
   Hence, any downtime for the sake of migration becomes less and less acceptable.
...
\section{Analysis}
\label{sct:Analysis}
   This section analyzes information systems, so we can formally define the migration procedure from one system to another.
...
\end{figure}
   ...
   In figure~\ref{fig:pre-migration} they are depicted in the schema s rules.
   We distinguish two types of constraint:
\begin{enumerate}
\item Invariant\\
   ...
   % Once it is restored, the system commits to the new state, which satisfies the constraint.
   ...
   A special case of an invariant is the \define{enforcement rule}.
   This corresponds to the classical database transaction.
   As soon as an enforcement rule is violated, the system restores it automatically, without human intervention,
...
\subsection{Zero downtime}
   ...
   Suppose we have an invariant, $u$, in the {\em desired system}, which is absent in the {\em existing system}.
   And suppose the data in the existing system does not satisfy $u$.
   When spinning up the desired system on the data set of the existing system, $u$ must be satisfied.
   If $u$ is an enforcement rule, the desired system will restore it automatically.
   But otherwise, the system will refuse to spin up.
   In many cases, clearing out all violations before spinning up the desired system might take too long, while new violations may arise in the meantime.
   So, the problem lies with invariants that are not enforced automatically.
   To solve this problem, we must implement such invariants initially as a business constraint,
   ...
   which implements the invariants of the desired system as business constraints, unless they are enforcement rules.
...
\end{enumerate}
   ...
   Ampersand is a compiler that generates information systems from a script.
   It uses a form of heterogeneous relation algebra,
   ...
   As a result, all invariants are explicit in the source code of a system specified in Ampersand.
...
\end{figure}
   ...
   The migration system has its own schema.
...
\end{enumerate}
   ...
   Before the migration, the data in the existing system satisfies its invariants (by definition).
...
%
   ...
   and expressions. We also define the difference of populations, both for relations and concepts:
\begin{align}
...
\end{equation}
   We represent an enforcement rule $u$ as a pair $\pair{\declare{n}{A}{B}}{\violC{u}}$,
   where relation $\declare{n}{A}{B}$ is used to restore invariance of the enforcement rule.
   The enforcement rule must be well-typed:
\begin{equation}
   \pair{a}{b}\in\viol{\pair{\declare{n}{A}{B}}{\violC{u}}}{\dataset}\ \Rightarrow\ a\instance A\ \wedge\ b\instance B
\end{equation}
   Besides, an enforcement rule must be automatically restorable by adding the violations to relation $\declare{n}{A}{B}$:
\begin{equation}
   \viol{u}{\dataset\cup\{\triple{a}{\declare{n}{A}{B}}{b}\mid\pair{a}{b}\in\viol{u}{\dataset}\}}=\emptyset
\end{equation}
   The language Ampersand has more types of enforcement rules than just this one,
...
\label{sct:Schemas}
   ...
   Error messages from the compiler are prohibitive for generating code,
   which prevents a substantial class of mistakes to ever reach end users.
   ...
   Every violation $\pair{a}{b}$ of a invariant $\triple{\violC{u}}{A}{B}$ must satisfy $a\instance A$ and $b\instance B$.
