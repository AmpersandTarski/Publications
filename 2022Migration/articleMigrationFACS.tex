...
\begin{enumerate}
\item Assume that $\infsys$ and $\infsys'$ are information systems, so they satisfy definition~\ref{def:information system}.
...
\item Prove that $\migrsys=\pair{\dataset_\migrsys}{\schema_\migrsys}$ is an information system,
      so the migration engineer can deploy the migration system.
\item Prove that $\overrightarrow{r}$ contains all triples from the existing data set in a finite time after deploying $\migrsys$,
