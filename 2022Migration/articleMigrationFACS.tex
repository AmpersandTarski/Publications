...
% \end{verbatim}
   \begin{align}
      ...
      \end{split}
   \end{align}
\item For each new blocking invariant $u$, we generate a helper relation: ${\tt fixed}_u$, and a transaction to produce its population:
% \begin{verbatim}
...
\item Let us combine the above into a single migration schema, $\schema_\migrsys$:
   ...
   \end{align}
\end{enumerate}
   After the migration system has been deployed, we must wait until the business has resolved all violations.
   This is something the system must signal, so the migration engineer can switch all traffic to the desired system:
% \begin{verbatim}
...
% \end{verbatim}
\begin{equation}
   \forall u\in\rules_{\schema'}-\rules_{\schema}.\ \viol{\overleftarrow{u}}{\dataset'}={\tt fixed}_u
\end{equation}
   After this, the migration system is no longer needed and can be removed.
