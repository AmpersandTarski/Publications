...
\end{figure}
   ...
   We assume that constraints implement the semantics of the data.
...
\item Transaction\\
   ...
   As soon as the data violates a transaction, the system restores it without human intervention.
...
\begin{align}
   ...
   u\in\rules\cup\transactions\wedge\sign{u}=\pair{A}{B}\wedge\pair{a}{b}\in\viol{u}{\dataset}&~\Rightarrow~a\instance A\ \wedge\ b\instance B\label{eqn:rule-has-type}\\
...
\end{align}
   ...
   Requirement~\ref{eqn:rule-has-type} ensures that violations in a rule correspond to the type of that rule. 
...
\begin{itemize}
   ...
   \item $\schema=\quadruple{\concepts}{\rels}{\rules}{\transactions}$ is a schema (satisfies equation~\ref{eqn:relationsIntroduceConcepts} thru~\ref{eqn:enforcement-has-type}), we write $\concepts_\infsys = \concepts$, $\rels_\infsys = \rels$, $\rules_\infsys=\rules$, and $\transactions_\infsys=\transactions$;
   \item All rules and transactions are satisfied:
   \begin{align}
   \forall u\in\rules&.~\viol{u}{\dataset}=\emptyset\\
   \forall (r\mapsfrom t)\in\transactions&.~\viol{u}{\dataset} - \pop{r}{\dataset}=\emptyset
...
\]
   ...
   So, the schema $\schema=\quadruple{\concepts}{\rels}{\rules}{\transactions}$ satisfies all requirements from section~\ref{sct:Schemas}.
...
\item We take a disjoint union of the relations, defining $\overleftarrow~$ and $\overrightarrow~$ as (overloaded) notation to generate new names, keeping the data set of the old system and the rules of the new system:
   ...
   \end{align}
\item We create transactions to copy equal relations, corresponding to equation~\ref{extraRelation} through~\ref{fill copies with new.isPartOf} in Section~\ref{sec:loading}:
   \begin{align}
   \widetilde{\declare{n}{x}{y}} &= \declare{(\id{copied}.n)}{x}{y}\\
   \rels_\theenumi &= \{\widetilde{r}\mid r \in \rels_\schema \cap \rels_{\schema'}\}\\
   \transactions_\theenumi &= \{\overrightarrow{r}\mapsfrom \popF{\overleftarrow{r} - \widetilde{r}} \mid \widetilde{r} \in \rels_2\} \cup \{\widetilde{r}\mapsfrom \popF{\overrightarrow{r}} \mid \widetilde{r} \in \rels_2\}
   \end{align}
\item For each rule, we generate two helper relations, {\tt viols} and {\tt fixed}. Note that Rule~\ref{warnings} only influences the user interface and therefore is not modeled here. This corresponds to equation~\ref{violRelation} through~\ref{rule uni}.
   ...
   \transactions_{\theenumi} &=\{{\tt viols}_u \mapsfrom \lambda \dataset'.~ \viol{u}{\overleftarrow{\dataset'}}\} \cup
...
\item Combining the above into a single migration system:
   ...
   \transactions_\migrsys &= \transactions_1\cup\transactions_2\cup\transactions_3
   \end{align}
\end{enumerate}
   
\subsection{Validation}
   We first observe that $\migrsys = \pair{\pair{\triples_\migrsys}{\instance_\migrsys}}{\quadruple{\concepts_\migrsys}{\rels_\migrsys}{\rules_\migrsys}{\transactions_\migrsys}}$ is a system after application of the transactions:
