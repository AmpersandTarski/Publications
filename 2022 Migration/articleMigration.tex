...
\subsection{Process of Data Migration}
   ...
   Let us first sketch the steps to migrate system $\infsys$ to $\infsys'$ informally.
\begin{enumerate}
   \item Let the existing system $\infsys$ have state $\dataset$ and schema $\schema$.
         ...
         If $\schema$ equals $\schema'$, we call the migration trivial because a data migration is not necessary.
   \item The generator generates a migration script $\migrsys$ based on $\schema$ and $\infsys'$
         ...
         It generates a schema $\id{migrationrules}$, which contains the enforce rules that migrate existing data to the desired dataset.
   \item A migration engineer changes $\id{migrationrules}$ to accommodate the non-standard requirements of the migration,
         ...
         This script also contains data structures and rules that are needed for the duration of the migration phase.
         ...
         As a result, the migration engineer can always restore the existing system.
   \item The production environment switches instantly from $\infsys$ to $\migrsys'$.
         This can be realized by configuring the ingress system to open the route to the migration system and the desired system,
   ...
   \item To ensure that all data is migrated before the the migration engineer makes the migration irreversible,
         we require that every rule from $\rules_{\migrsys'}$ is satisfied before the migration engineer takes down the existing system.
         Users (i.e.\ the business) must do the work to satisfy the manually enforced rules.
         Ampersand guarantees that enforce rules will be satisfied.
   \item To give the business an opportunity to evaluate the change,
         ...
         In case of a go-decision, the migration engineer can disable the undo-facility as soon as the migration rules are satisfied.
         In case of a no-go, the migration engineer restores the existing system.
         This event marks the moment of no return (MoN).
   \item The system retains the data, the schema, and the functionality that is covered by $\schema'$ by dropping everything else.
...
\begin{definition}[relabel relations]
   \[\subst{\declare{n}{A}{B}}{r}{{\declare{m}{C}{D}}}\ =\ \text{\bf if}\ n=m\wedge A=C\wedge B=D\ \text{\bf then}\ r\ \text{\bf else}\ \declare{m}{C}{D}\]
\end{definition}
\begin{definition}[relabel relations in triples]
   \[\subst{p}{q}{\triple{a}{r}{b}} = \triple{a}{\subst{p}{q}{r}}{b}\]
\end{definition}
\begin{definition}[relabel relations in datasets]
   \[\begin{array}{rcl}
      \rlap{$\subst{p}{q}{\pair{\triples}{\inst}}$}\\
      &=&\pair{\{\subst{p}{q}{t}\mid t\in\triples\}}{\inst}
   \end{array}\]
\end{definition}
\begin{definition}[relabel relations in rules]
   \[\sat{u}{\dataset}\ \Leftrightarrow\ \sat{\subst{p}{q}{u}}{\subst{p}{q}{\dataset}}\]
\end{definition}
\begin{definition}[relabel relations in schemas]
   \begin{eqnarray}
      \rlap{$\subst{\declare{n}{A}{B}}{{\declare{m}{C}{D}}}{\triple{\concepts}{\rels}{\rules}}$}\notag\\
      &=&\begin{array}[t]{l@{}l}
         \la&\{\subst{B}{D}{{\subst{A}{C}{c}}}\mid c\in\concepts\}\\
         ,&\{\subst{B}{D}{{\subst{A}{C}{{\subst{\declare{n}{A}{B}}{{\declare{m}{C}{D}}}{r}}}}}\mid r\in\rels\}\\
         ,&\{\subst{B}{D}{{\subst{A}{C}{{\subst{\declare{n}{A}{B}}{{\declare{m}{C}{D}}}{u}}}}}\mid u\in\rules\}\ \ra\notag
         \end{array}
   \end{eqnarray}
\end{definition}
