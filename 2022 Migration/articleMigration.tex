...
%    Launch the desired system in parallel to the old one, copy data from the old to the desired system, and have everyone use the desired system.
%    However, numerous issues might impede that plan.
%    The following examples illustrate the practical issues that may occur:
% \begin{enumerate}
% \item Data required in the desired system is missing in the old system.
...
% \item Data in the old system does not satisfy invariants of the desired system.
...
% \item A way of entering data into the old system is missing in the desired system.
...
% \item Data present in the old system cannot be stored in the desired system.
...
%    \item Allow continued use of interfaces of the old system, data entered into the old system via those interfaces needs to be continuously copied to the desired system.
...
\begin{enumerate}
   ...
   %       Automatic migration rules can be executed at the MoT.
   %       Migration rules that are enforced manually will be held alive until they are satisfied.
   %       All rules from $\schema$ (the rules from the old system) are removed on the MoT, because the are satisfied by property~\ref{eqn:satisfaction}.
