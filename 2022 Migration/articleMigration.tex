...
\begin{figure}[bht]
   \begin{center}
     \includegraphics[scale=.45]{figures/datamigration-Pre-migration.drawio.png}
   \end{center}
...
\begin{figure}[bht]
   \begin{center}
     \includegraphics[scale=.35]{figures/datamigration-Migration phase.drawio.png}
   \end{center}
...
\begin{figure}[bht]
   \begin{center}
     \includegraphics[scale=.35]{figures/datamigration-Post-migration.drawio.png}
   \end{center}
...
\begin{enumerate}
   \item Let $\infsys$ have data set $\dataset$ and schema $\schema$.
         A developer defines $\infsys'$, which contains a data set of its own, $\dataset'$, a schema $\schema'$, and some functionality\footnote{For now, we ignore the functionality.} as illustrated in figure~\ref{fig:post-migration}.
   ...
   \item The migration engineer uses a software generator to generate a migration script, $\migrsys$,
         ...
         It contains enforce rules that migrate existing data to the desired data set.
   ...
   \item\label{item3}
         ...
         The migration engineer does not alter $\schema$ equals $\schema'$ while adapting $\migrsys'$ to the specific needs of the current migration.
   \item Then, the migration engineer deploys system $\migrsys'$.
         ...
         both data sets are in sync with respect to the ongoing transactions.
   \item At this point, the ingress switches the traffic from $\infsys$ to $\migrsys'$,
         ...
         From this point onward, users work with the desired functionality in data set $\dataset'$.
...
\end{eqnarray}
   ...
   An enforce rule with operator {\tt :=} (alternative~\ref{enforce eq}) keeps the population of $r$ equal to \id{expression} by inserting and deleting the right pairs in $r$.
