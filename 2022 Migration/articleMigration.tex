...
\begin{enumerate}
   \item Let the existing system $\infsys$ have state $\dataset$ and schema $\schema$.
         ...
         After the migration, the system must contain the triples from $\dataset$ and $\dataset'$, to preserve the existing data.
   ...
   \item The production environment switches instantly from $\infsys$ to $\migrsys'$,
         ...
         From this point onward, both the existing functionality and the desired functionality are available to users.
   ...
   %       All rules from $\schema$ (the rules from the existing system) are removed on the MoT, because the are satisfied by property~\ref{eqn:satisfaction}.
   \item After a finite span of time, $\dataset_{\migrsys'}$ satisfies all its enforce rules.
         This implies that the generated rules plus the changes made by the migration engineer ensure the preservation of the meaning of the existing data.
   ...
   \item The patron of the system can decide whether to carry on with the migration or to roll back to the existing system.
