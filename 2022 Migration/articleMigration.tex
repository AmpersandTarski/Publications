...
% \end{eqnarray}
%    To determine whether rule $u$ satisfies dataset $\dataset$,
%    we define $\sat{u}{\Dataset}$:
% \begin{eqnarray}
%    \sat{u}{\dataset}&\Leftrightarrow&\viol{u}{\dataset}=\emptyset
%    \label{eqn:sat}
% \end{eqnarray}
%    Violations are used to produce meaningful error messages and to trigger changes in the dataset to restore invariance of that rule.
...
\end{definition}
   ...
   The expression $o\ \maintain\ u$ means that role $o$ is responsible for maintaining rule $u$, i.e. that rule $u$ is kept satisfied at all times.
...
\end{eqnarray}
   ...
   Requirement~\ref{eqn:satisfaction} ensures that the dataset satisfies all rules that are not maintained by a role.
%    The following is for wide tables only
...
\]
   ...
   This meets all requirements for $\infsys=\quadruple{\dataset}{\schema}{\roles}{\maintain}$ from definition~\ref{def:information system}.
...
\label{sct:Changing data}
   Every rule in the schema represents a constraint on the dataset.
   The rule serves as invariant,
   which means that the data in the database must satisfy the rule at all times.
   If an actor tries to change the dataset in a way that violates this rule,
   the system will either reject that change or help the actor to find additional changes to keep it satisfied.
   The phrase ``restore an invariant'' or ``restore invariance of a rule''
   means that an actor changes the dataset to satisfy the constraint that rule represents.
