...
\begin{equation}
   \pop{r}{\dataset}\ =\ \{ \pair{a}{b}|\ \triple{a}{r}{b}\in\dataset\}
\end{equation}
...
\label{sct:Schemas}
   ...
   For every rule $u$ in the schema, the Ampersand compiler generates code for the predicate $\id{sat}_u\{\Dataset\}$
...
\begin{itemize}
   \item dataset $\dataset$ and $\schema$ are defined as in sections~\ref{sct:Schemas} and~\ref{sct:Datasets};
...
\begin{equation}
   \forall a,r,b:\ \triple{a}{r}{b}\in\dataset\ \Rightarrow\ r\in\rels
\end{equation}
...
\begin{eqnarray}
   ...
   \forall\triple{a}{\declare{n}{A}{B}}{b}\in\dataset&:&a\ \inst\ A\ \wedge\ b\ \inst\ B
\end{eqnarray}

   To compare atoms for equality,
   the set $\rels$ must contain an identity relation $\ident{C}$ for every concept $C$ in which all atoms occur.
   It must satisfy:
\begin{equation}
...
\begin{eqnarray}
   \forall C\in\concepts&:&\pop{\ident{C}}{\dataset}=\{\pair{a}{a}| a\ \inst\ C\}
\end{eqnarray}
...
\begin{equation}
   ...
   \id{sat}_{A\isa B}\{\dataset\}\ \Leftrightarrow\ \forall a: a\ \inst\ A\rightarrow a\ \inst\ B
\end{equation}
   ...
   That role must ensure to satisfy that rule, after some event has violated the rule by changing the set of triples.
   ...
   $o\ \maintain\ u$ means that role $o$ must keep the set $\viol{u}{\dataset}$ empty.
   If a rule $u$ has violations, i.e. $\viol{u}{\dataset}$ is not empty, there is a role whose job it is to remove those violations.
...
% The purpose of this section is to explain why a dataset is structured the way it is.
   ...
   To satisfy definition~\ref{def:dataset}, $\dataset$ and $\atoms$ must change too,
