...
\begin{verbatim}
   RELATION viol_EnrollRule[Student*Module]
   RELATION diff_takes[Student*Course]
   RELATION diff_isPartOf[Module*Course] [UNI]
   RELATION diff_isEnrolledFor [Student*Module]

   ENFORCE diff_takes :< old.takes-new.takes
 
  -- ENFORCE new.takes >: old.takes  - diff_takes
  -- ENFORCE new.takes :< old.takes \/ diff_takes
   ENFORCE old.takes >: new.takes  - diff_takes
   ENFORCE old.takes :< new.takes \/ diff_takes
   
   ENFORCE new.isPartOf := old.isPartOf
   ENFORCE new.isEnrolledFor := old.isEnrolledFor
\end{verbatim}
   The prefix `old.` refers to concepts, relations, and rules from the existing information system, $\infsys$, which is defined in section~\ref{sct:Example existing IS}.
   The prefix `new.` refers to items from the desired information system, $\infsys'$, defined in this section.
   The population just before the moment of transition is.
\begin{verbatim}
   RELATION takes[Student*Course]
   RELATION isPartOf[Module*Course] [UNI]=
