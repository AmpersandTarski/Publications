...
\label{sct:Introduction}
   ...
   Every event that an actor causes in information system $\infsys$ may change the state $\dataset$.
   We will write $\dataset_{\infsys}$ in case we need to be specific about which information system the dataset belongs to,
   but just $\dataset$ if it is obvious or irrelevant to which information system this dataset belongs.

   In this work we will encode the semantics in a schema,
   so we can be explicit about ``preserving the meaning as much as possible''.
   The schema of an information system is basically a set of constraints on its dataset $\dataset$.
   We will denote the schema of an information system $\infsys$ by $\schema_{\infsys}$,
   or just $\schema$ if it is obvious or irrelevant to which information system this schema belongs.
   ...
   Our theory is based on the following premises:
\begin{itemize}
