...
\label{sct:Datasets}
   ...
   Where ambiguity lures, we write $\triples_{\dataset}$ and $\inst_{\dataset}$ to indicate that we specifically mean the dataset $\dataset$.
...
\begin{enumerate}
   ...
   \item The generator generates a migration system $\migrsys$ based on $\schema$ and $\schema'$,
         ...
         i.e.\ $\schema_{\migrsys}=\schema\sqcup\schema'$.
   \item A migration engineer changes $\migrsys$ to accommodate the non-standard requirements of the migration,
         yielding migration system .
   ...
   \item The system removes the data, the schema, and the functionality of $\migrsys'$ that are not supported by $\schema'$.
         i.e\ $\schema_{\migrsys''}=\schema'$ and $\dataset_{\migrsys''}=\select{\schema'}{\dataset_{\migrsys''}}$.
\end{enumerate}
