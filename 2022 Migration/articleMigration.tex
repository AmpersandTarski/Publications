...
\begin{enumerate}
   \item Let the existing system $\infsys$ have state $\dataset$ and schema $\schema$.
         ...
         After the migration, the system must contain the triples from both $\dataset$ and $\dataset'$, to preserve the existing data.
   ...
   \item The migration system $\dataset_{\migrsys'}$ must be at rest before the MoN,
         to ensure that the migration has completed before the migration becomes irreversible.
