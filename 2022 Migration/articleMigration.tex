...
\label{sct:Introduction}
   We believe in generating evolving systems because it addresses human errors.
   By generating information systems rather than programming them by hand, we avoid introducing errors in the translation from design to concrete system.
   ...
   We recognise that even designs can have errors in them:
   either the error was made directly by a designer, the requirements the designer was taking into account were miscommunicated, or the reality changed in a way was not foreseen at the time the design was made.
   Regardless of the reason, even a generated information system may need to be repaired.
   For this reason, we focus on evolving systems.
   
   When a generated system evolves, we are left with the problem of migrating the data from the concrete old system to the newly designed system.
   This situation poses specific requirements to data migration.
   For instance, if a data migration is not for evolving systems,
   ...
   For evolving a system, however, preserving the functionality is not an option.
   To change the functionality is the very purpose of the evolution, so the data migration must support just that.
   So, we must make sure to use methods for data migration that are specifically meant for evolving information systems.
