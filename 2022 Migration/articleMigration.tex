...
\end{itemize}
   ...
   These new pairs are copied by the same rule.
   However, {\tt old.takes} may also lose pairs, which are 
   Then, the enforce rule that fills {\tt new.takes} will be satisfied forever and will change nothing more to {\tt new.takes}.

   But what if (in the meantime) someone removes a pair from {\tt old.takes} that has already been inserted in {\tt new.takes}?
\begin{verbatim}
   RELATION added_takes[Student*Course]
   RELATION deleted_takes[Student*Course]
   ENFORCE new.takes >: old.takes - added_takes
   ENFORCE added_takes >: new.takes
\end{verbatim}
...
\subsection{Migration system}
   The generated migration system, $\migrsys$, is:
\begin{verbatim}
...
\subsection{Preparing for the Moment of Transition}
   ...
   the data in the existing system is no longer the data we saw in section~\ref{sct:Example existing IS}.
