...
\begin{abstract}
   Incremental software development with a software generator can save work and reduce the time to deploy changes,
   but data migrations that change the database schema can be challenging.
   The problem is how to preserve the semantics of the data under a changing schema.
   
   This paper proposes a theory for deploying an incremental change in an already generated system in production using the Ampersand project.
   Our approach aims to preserve data semantics and satisfy elementary business requirements
   while featuring zero down-time and an undo option after deployment.
...
\begin{definition}[union of datasets]
...
\begin{definition}[map relation names in datasets]
