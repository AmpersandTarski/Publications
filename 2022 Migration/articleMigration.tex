...
\label{sct:Datasets}
   ...
   This relation is used in the type system, in which $\inst$ assigns a type to every atom.
   This makes $\inst$ total on the atoms in the dataset:
\begin{eqnarray}
...
\end{eqnarray}
   ...
   A rule in a schema serves to constrain the dataset, to ensure its semantic integrity.
   ...
   We also assume a function $\Sign : \rules\rightarrow\Pair{\Concepts}{\Concepts}$
   that represents the signature of a rule, i.e. the type of atoms in violations:
\begin{eqnarray}
   \sign{u}=\pair{A}{B}\\
   \pair{a}{b}\in\viol{u}{\dataset}&\Rightarrow&a\ \inst\ A\wedge b\ \inst\ B
...
\end{equation}
   ...
   The specialization rules in a schema form a partial order $\isa : \Pair{\concepts}{\concepts}$,
...
\end{equation}
   ...
   which specifies not only the rule, but also the way to restore invariance.
   The system contains an engine that restores invariance of all automatically enforced rules without unneccessary delay.
   Whenever the dataset violates an enforce rule,
   this engine will make other changes to the dataset to satisfy that rule.
   Hence, the violation of the rule exists during a small, finite amount of time.
   ...
   (\verb-@sjcjoosten-, what if $\maintain$ relates both a human role and the enforcement engine role to the same rule?)
...
\end{verbatim}
   We also define a rule, {\tt EnrollRule}, that states that a student can enroll for any module that is part of a course that a student takes.
...
\end{verbatim}
   The semantics of Ampersand defines $\viol{\tt EnrollRule}{\dataset}$ as follows:
\begin{equation}
   \viol{\tt EnrollRule}{\dataset}\ =\{\begin{array}{l}
   \pair{m}{s}\mid\neg\exists c\in\text{\tt Course}:\\
s\ \text{\tt isEnrolledFor}\ m\ \rightarrow\ s\ \text{\tt takes}\ c\ \wedge\ m\ \text{\tt isPartOf}\ c
\end{array}\}
\label{eqn:example isEnrolledFor}
\end{equation}

   This example contains a dataset with seven triples
   ...
   \end{array}\]
   Requirement~\ref{eqn:typedAtom} is satisfied because every atom has a concept.
   The tuple $\pair{\triples}{\inst}$ satisfies both requirements~\ref{eqn:wellTypedEdge} and~\ref{eqn:pop}.
   So, this defines a dataset $\dataset=\pair{\triples}{\inst}$ as introduced in section~\ref{sct:Datasets}.
   ...
   $\rules$ satisfies requirement~\ref{eqn:satisfaction} because $\viol{\tt EnrollRule}{\dataset}$ is empty.
   So, the schema $\schema=\triple{\concepts}{\rels}{\rules}$ satisfies the requirements from section~\ref{sct:Schemas}.

   To fulfill the definition of information system, definition~\ref{def:information system},
   ...
   The constraint~\ref{eqn:example isEnrolledFor} of rule {\tt EnrollRule} is satisfied
   because relation $\declare{\tt isEnrolledFor}{\tt Student}{\tt Module}$ is empty.
   So, there are no violations, which satisfies requirement~\ref{eqn:wellTypedEdged violations}.
   With only one rule in the system, requirement~\ref{eqn:sat} is satisfied too.
   Requirement~\ref{eqn:specialization} is satisfied because this example contains no specialization.
   This completes the requirements for $\la\dataset,\schema,\roles,\maintain\ra$.
...
% \begin{eqnarray}
%    \forall a\in\atoms\ \exists A\in\concepts&:&a\ \inst\ A\label{eqn:typedAtom}
% \end{eqnarray}
...
% \begin{eqnarray}
%    \forall\declare{n}{A}{B}\in\rels&:&A\in\concepts\ \wedge\ B\in\concepts\label{eqn:typedAtom}\\
%    \forall\triple{a}{\declare{n}{A}{B}}{b}\in\triples&:&a\ \inst\ A\ \wedge\ b\ \inst\ B\label{eqn:wellTypedEdge}
% \end{eqnarray}
