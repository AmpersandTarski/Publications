...
\begin{eqnarray}
   ...
   \isa&&\text{is a partial order on $\concepts$}
   \label{eqn:isasPartialOrder}\\
   \forall u\in\rules,\exists\dataset\in\Dataset-\emptyset&:&\sat{u}{\dataset}
   \label{eqn:trueifyableRules}
\end{eqnarray}
   Note that these requirements do not depend on any particular dataset,
   so they can be checked statically.
...
\end{eqnarray}
   ...
   The rules in the schema serve as invariants,
...
\label{sct:Changing data}
   ...
   We will use variable $e$, $e'$, $e''$, and so on to represent events.
...
\end{eqnarray}
   ...
   This may violate requirement~\ref{eqn:satisfaction} when one or more rules are broken,
   i.e.\ if there is a rule $u$ for which $\sat{u}{e(\dataset)}$ does not hold.
   In that case, requirement~\ref{eqn:satisfaction} prohibits the system to be in state $e(\dataset)$,
   so either the system stays in the state $\dataset$ or there is a reaction $e'$ that restores invariance of the broken rules:
\begin{equation}
...
\end{equation}
   ...
   In practice, there are different ways of enforcing satisfaction of rules.
   One way is to let a user or a computer ``invent'' $e'$.
   A developer can specify this by assigning a rule $u$ to a role $o$.
   Any actor with role $o$
...
\end{equation}
   ...
   If $\maintain$ specifies no role for a given rule $u$, the system can prevent $e$ from having any effect.
