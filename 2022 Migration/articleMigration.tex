...
\begin{eqnarray}
   ...
   \pair{A}{B}\isa\pair{C}{D}\ \wedge\ \declare{n}{C}{D}\in\rels&\Rightarrow&
   \begin{array}[t]{@{}l}
      \declare{n}{A}{B}\in\rels\ \wedge\\
      \pop{\declare{n}{A}{B}}{\dataset}\subseteq\pop{\declare{n}{C}{D}}{\dataset}
   \end{array}
...
\end{eqnarray}
   ...
   to restore invariance.
   In the Ampersand language, a developer explicitly assigns a rule $u$ to a role $o$,
   to keep control over the type of persons that are allowed to ``break rules temporarily''.
   An actor, who attempts an event that violates a rule, should receive notification of the system with an appropriate error message.
