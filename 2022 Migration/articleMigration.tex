...
\title{It's all about Meaning!\\{\normalsize A Theory for Data Migration when Deploying Changes}}
...
\begin{abstract}
   ...
   and features an undo after verification by the business.
   It also leaves room for the business to finish ongoing work
   after the moment the change has been deployed.
\end{abstract}

\begin{keyword}
generative software\sep incremental software development\sep data migration\sep relation algebra \sep Ampersand
\end{keyword}
\maketitle
...
\begin{figure}[bht]
   \begin{center}
     \includegraphics[scale=.35]{Migration.drawio.png}
   \end{center}
...
\end{figure}
   ...
   a migration engineer sometimes has to make choices.
   If the interpretation of data changes,
   a migration engineer drops obsolete constraints from the old system and introduces new constraints in the new system.
...
\subsection{Data migration}
   ...
   This section studies the transition from an old system to a new one, involving a data migration.

   In a datamigration, we distinguish three moments:
\begin{enumerate}
   \item the moment of transition, i.e. the moment the old system is replaced (in production) by the new system;
   \item the decision point, i.e. the moment a business authority declares the migration successful;
   \item the point of no return, i.e. the moment that the migration cannot be undone other than by a backup-procedure,
         which is outside the scope of this paper.
\end{enumerate}
   The decision point must come after the moment of transition, to allow the business to verify the migration.
   The point of no return must come after the decision point, to allow for a no-go decision from the business.
...
\begin{enumerate}
   \item Let the old system $\infsys$ have state $\dataset$ and schema $\schema$.
         A developer defines a new system $\infsys'$, which contains a dataset $\dataset'$, a schema $\schema'$, and some functionality\footnote{For now, we ignore the functionality.}.
         In Ampersand, this means that the developer produces an Ampersand script for the new system.
         The {\em increment} is the difference between the new system and the old system.
         The new system must contain the triples from $\dataset$ and $\dataset'$, to preserve the old data.
         If the developer wants to add some new data into the system on the moment of transition,
   ...
   \item The generator generates a migration system $\migrsys$ based on $\schema$ and $\infsys'$,
   ...
   \item A migration engineer changes $\id{migrationrules}$ to accommodate the non-standard requirements of the migration,
         ...
         The migration engineer does not alter the old system $\infsys$ nor the specification of the new system $\infsys'$ that the developer has produced.
   ...
   \item The production environment switches instantly from $\infsys$ to $\migrsys'$,
         ...
         This event marks the moment of transition.
   \item The rules in the migration system that are not in $\schema'$ will remain in the system until they are satisfied,
         to enforce their execution.
         Automatic migration rules can be executed at the moment of transition.
         Migration rules that are enforced manually will be held alive until they are satisfied.
         All rules from $\schema$ (the rules from the old system) are removed on the moment of transition, because the are satisfied by property~\ref{eqn:satisfaction}.
