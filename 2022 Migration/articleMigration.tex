...
\begin{abstract}
   ...
   to transfer the data and preserve their semantics as much as possible.
...
\begin{enumerate}
   ...
   \item the point of no return, i.e. the moment that the migration cannot be undone other than by the regular backup-procedure.
\end{enumerate}
...
\begin{enumerate}
   \item Let the old system $\infsys$ have state $\dataset$ and schema $\schema$.
         A developer defines a new system $\infsys'$, which contains a dataset $\dataset'$, a schema $\schema'$, and the functionality\footnote{For now, we ignore the functionality.}.
         In Ampersand, this means that the developer produces an Ampersand script for the new system.
         The difference between the new system and the old system is the increment.
         To preserve the semantics of data means that the dataset $\dataset$ must be combined with $\dataset'$ in the new system.
         Dataset $\dataset'$ is optional, in case the developer wants to add some new data into the system on the moment of transition.
   ...
   \item A migration engineer changes $\id{migrationrules}$ to accommodate the non-standard requirements of the migration,
         ...
         The migration engineer does not alter the old system $\infsys$ nor the specification new system $\infsys'$ that the developer has produced.
...
\begin{definition}[relabel concepts in $\inst$]
   \[a\ \subst{C}{D}{\inst}\ A\ =\ \text{\bf if}\ C=A\ \text{\bf then}\ a\ \inst\ D\ \text{\bf else}\ a\ \inst\ A\]
\end{definition}
