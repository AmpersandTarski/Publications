...
\label{sct:Datasets}
   ...
   This relation is used in the type system, in which $\inst$ assigns a concept to every atom in the dataset.
...
\end{equation}
   ...
   in which a user defines a pair of concepts in the relation $\isa$.
   A compiler can construct $\isa$ as the transitive, reflexive closure of such user-defined pairs of concepts.
   It must test for antisymmetry, ensuring that $\isa$ is a partial order of concepts.
   Specialization causes atoms to have multiple concepts of which they can be an instance.
   To ensure that this does not result in problems at run-time, a compiler can require users to declare all specializations explicitly.
   ...
   A dataset $\dataset$ is a structure $\triple{\triples}{\isa}{\inst}$ that satisfies:
\begin{eqnarray}
   \triple{a}{\declare{n}{A}{B}}{b}\in\triples&\Rightarrow&
      \begin{array}[t]{@{}l}
         \exists C: C\isa A\ \wedge\ a\ \inst\ C\ \wedge\\
         \exists D: D\isa B\ \wedge\ b\ \inst\ D
      \end{array}
...
%    Violations are used to produce meaningful error messages and to trigger changes in the dataset to restore satisfaction of that rule.
   ...
   A schema $\schema$ is a structure $\quadruple{\concepts}{\isa}{\rels}{\rules}$,
...
\item An information system $\infsys$ is a structure $\la\dataset,\schema,\roles,\maintain\ra$, in which
\begin{itemize}
   \item dataset $\dataset=\triple{\triples}{\isa}{\inst}$ is defined as in section~\ref{sct:Datasets};
...
\end{itemize}
\end{definition}
   Note that dataset $\dataset$ and schema $\schema$ have the same specialization relation $\isa$.
...
\end{equation}
   ...
   The system features an engine that restores invariance of all enforce rules without unneccessary delay.
