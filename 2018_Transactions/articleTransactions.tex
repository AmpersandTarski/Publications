...
\section{Traces}
\label{sct:Traces}
	We need the notion of {\em event}\footnote{Cambridge Dictionary defines ``event'' as: anything that happens} to describe things that happen in a distributed information system.
	Being distributed means that we must assume that the temporal order of events is partial:
	We do not necessarily know in which order two events have happened.
	To describe this, we define traces algebraically corresponding to Foster~\cite{Foster17b}.

	We define traces as a set ${\mathcal T}$ with
	a concatenation operator $+:\mathcal T\rightarrow\mathcal T\rightarrow\mathcal T$ and
	the empty trace $0:\mathcal T$, which obey the following axioms.
\begin{definition}[Trace Algebra]
\label{def:Trace Algebra}
\item   A trace algebra $(\mathcal T, +, 0)$, is an algebra that satisfies: 
\begin{eqnarray}
	x+(y+z)&=&(x+y)+z\label{axi:associative}\\
	0+x&=&x+0\label{axi:lr-identity}\\
	x+y\ =\ x+z&\Rightarrow&y=z\label{axi:right cancellation}\\
	x+z\ =\ y+z&\Rightarrow&x=y\label{axi:left cancellation}\\
...
\end{definition}
	...
	Equations~\ref{axi:right cancellation} and~\ref{axi:left cancellation} state that $+$ is {\em cancellative}.
	Equation~\ref{axi:no inverse} makes the cancellative monoid into a trace algebra.
	It implies that there is no inverse.

	In terms of information systems, a trace represents history.
	For that purpose it makes sense to define the notion of prefix, so we can define notions like shared history.
...
\section{History}
\label{sct:History}
	The history of a distributed information system (DIS) consists of events and an ordering on these events.
	Because a DIS is distributed,
	we may not know the temporal order of two events in the history structure.
	For this reason we describe the history structure of a DIS as a partial order of events.
...
\end{eqnarray}
	...
	For this reason we have requirement~\ref{req:precPlus asy}, which says that the transitive closure $\id{prec}^+$ must be antisymmetric.
	...
	Function $\id{current}:\mathbb H\rightarrow\powerset{\Events}$ computes the current events in a history structure.
	Events in $\Events$ that have no preceding events are called {\em initial} events.
	Function $\id{past}$ computes the past events and $\id{initial}$ computes the initial events.
\begin{eqnarray}
	\id{current}(\la \Events, \id{prec}\ra)&=&\{ c\in\Events|\ \neg\exists e_1\in\Events: e\ \id{prec}\ e_1 \}\\
	\id{initial}(\la \Events, \id{prec}\ra)&=&\{ c\in\Events|\ \neg\exists e_1\in\Events: e_1\ \id{prec}\ e \}
\end{eqnarray}
	We define the {\em closure} of a history structure as the history structure whose order is the reflexive, transitive closure of the original ordering relation.
\begin{eqnarray}
	\la \Events, \id{prec}\ra^*&=&\la \Events, \id{prec}^*\ra\label{def:closure}
\end{eqnarray}
	Equation~\ref{def:closure} allows us to write $h^*$ for elements of $\mathbb H$.
	The order of $h^*$ is reflexive, transitive, and antisymmetric.
	...
	We use $h\subseteq h'$ to state that $h$ lies in the past of $h'$.
	Let $h=\la \Events, \id{prec}\ra$ and $h'=\la \Events', \id{prec}'\ra$ in the definition of $h\subseteq h'$:
\begin{equation}
\label{def:lies in the past}
	h\subseteq h'\ \Leftrightarrow\ 
	\Events\subseteq\Events'\ \wedge\ \id{initial}(h)\subseteq\id{initial}(h')\ \wedge\ \id{prec}^*\subseteq\id{prec}'^*
\end{equation}
