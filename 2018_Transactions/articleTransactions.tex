...
\label{sct:Introduction}
	...
	As a result, many architectural patterns have emerged for the purpose of doing transactions in a distributed setting.
	Some of these patterns simply advise how to avoid or circumvent the problem.
	Others will isolate transactionality into a smaller sub-problem,
        downsizing the problem and hoping to deal with the sub-problem in a conventional transactional database.
	Still others introduce locking mechanisms to do the job.
        However, sometimes we must avoid locking for performance reasons and avoid a globally shared database for reasons of size.
        As is the case with large, globally distributed information systems such as Facebook, LinkedIn, and Netflix.
        In case we want full transactional behaviour in such systems, we have a problem.

        This problem seems to be solved by Git.
	Our work is inspired by the observation that the merge of Git corresponds to a transactional commit.
	...
	For expressing transactions in terms of events we have to redefine Atomicity, Consistency, Isolation, and Durability.
...
We define the history of a distributed information system as a directed acyclig graph $\la\mathcal E,\mathcal C\ra$ with
\begin{definition}
\begin{eqnarray}\mathcal C\subseteq\mathcal E\times\mathcal E\\
\mathcal C^+\cap I_{\mathcal E}=\emptyset
\end{eqnarray}
\end{definition}
A transaction is a set of events.
\begin{enumerate}
\item   Atomic\\
A transaction is atomic if it can be replaced by a single event without consequences for other parts of the system.
\item   Consistent\\
If the initial states of a transaction satisfy a given set of rules, we say that a transaction maintains consistency if its final state satisfies that same set of rules.
\item   Isolated\\
A transaction is isolated if its events are not shared by other transactions.
\item   Durable\\
A transaction is durable if it is part of every future history.
\end{enumerate}
...
\section{State}
\label{sct:State}
	Typically, distributed information systems may have states.
%        Consider for example Twitter: the entire timeline on your smartphone represents the state.
	Every new event in the history of a DIS may cause a different state.
	The distributed nature of a DIS means that states are local.
	Let $\mathbb S$ be the set of all conceivable states.
	We need a function $\id{state}:\mathbb E\rightarrow\mathbb S$ that assigns one state to every event $e$ in history $\id{prec}$.
	...
	The global state of the system, if deemed relevant, can be computed by $\{\id{state}(e) | e\in\id{current}(\id{prec})\}$.
