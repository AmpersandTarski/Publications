...
\label{sct:State}
	...
	So we need a function $\id{state}_\id{prec}:\mathbb E\rightarrow\mathbb S$ that assigns one state to every event $e$ in history $\id{prec}$.
	State $\id{state}_\id{prec}(e)$ represents the state immediately after $e$ has happened.
	If the history $\id{prec}$ is obvious from the context, we may write $\id{state}(e)$ rather than $\id{state}_\id{prec}(e)$.
...
\end{equation}
	...
	We start with the easiest one: durability.
	This property expresses that an action must preserve history.
\begin{definition}[Durability]
\item An action $\id{act}_{e_1,e_2,\Delta}$ is durable iff:
...
\end{array}\]
\end{definition}
	Note that durability is already satisfied in the definition of action (equation~\ref{def:action}). (proof needed)
	Such is not the case with isolation.
	An action $\id{act}_\id{prec}(e_1,e_2)$ is isolated, i.e. performed in isolation, if it does not affect any other action in the system.
...
\item An action $\id{act}_{e_1,e_2,\Delta}$ works isolated in history $\id{prec}$ iff:
\label{def:Isolation}
\begin{eqnarray}
...
\end{definition}
	...
	Atomicity is related to isolation because it expresses that a sequence of multiple actions can be treated as one.

\begin{definition}[Atomic]
\item An action $\id{act}_{e_1,e_2,\Delta}$ is atomic in history $\id{prec}$ iff:
\[\begin{array}{rl}
	&\id{act}_{e_1,e_2,\Delta}(\id{prec},\delta_\id{prec})\ =\ (\id{prec}',\delta'_{\id{prec}'})\\
	\Rightarrow\\
