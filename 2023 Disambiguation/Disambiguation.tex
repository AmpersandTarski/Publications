...
\mathrm{before}_{\mathrm{Task * Task}} \typeinter \typeident_\mathrm{Task} &\typesubset \typenil_\mathrm{Task * Task}
\end{align}

The intended meaning behind these invariants is: 1) The set of tasks that needs to be performed before some task $x$, all need to be scheduled at times before the time at which $x$ itself is scheduled. 2) It makes no sense to require a task to be performed before itself.
...
For the other expression, we cannot omit writing all types: $\mathrm{before} \typeinter \typeident \typesubset \typenil$ could be interpreted as:
$\mathrm{before}_{\mathrm{Time * Time}} \typeinter \typeident_{\mathrm{Time}} \typesubset \typenil_{\mathrm{Time * Time}}$.
To avoid this issue, one might write \emph{some} type information: $\mathrm{before} \typeinter \typeident \typesubset \typenil_{\mathrm{Task * Task}}$.
From this, it should become clear which relations were intended (namely $\mathrm{before}_\mathrm{Task * Task}$ and $\typeident_\mathrm{Task}$), even though they appear elsewhere.
...
To illustrate why we prefer to use $\varepsilon_{A*B}$ relations where $A=B$, consider the expression: $(\mathrm{before} \typeinter \typeident) \typeunion \typetyped{\typenil}{\mathrm{Task * Task}}$. This expression is similar to one of the terms from our running example, but written as a proper $\mathcal{S}$-term.
Possible disambiguated terms include:
\begin{itemize}
\item $\varepsilon_T;(\varepsilon_T;\mathrm{before}_T;\varepsilon_T \cap \varepsilon_T;\typeident_\mathrm{Task};\varepsilon_T);\varepsilon_T \cup \varepsilon_T;F_T;\varepsilon_T$ with $T = \mathrm{Task*Task}$
\item
\item
\end{itemize}
